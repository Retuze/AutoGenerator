...
# LastEditors   : Retuze 
# LastEditTime  : 2024-01-09 04:41:47
# Description   :
*********************************************/
#include "u_printf.h"
...
// 字符串长度
static unsigned long long u_strnlen(const char *s, unsigned long long count)
{
    const char *sc;
...
// 字符转10进制数
static int u_skip_atoi(const char **s)
{
    int i = 0;
    while (is_digit(**s))
        i = i * 10 + *((*s)++) - '0';
...
//
static char *u_number(char *str, long num, int base, int size, int precision, int type)
{
    char c, sign, tmp[66];
...
// 计算浮点精度
static int u_ilog10(double n) /* 在整数输出中计算log10(n) */
{
    int rv = 0;
...
// 整数位数
static double u_i10x(int n) /* 计算10^n的整数输入 */
{
    double rv = 1;
...
)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (fmt == 'f')
            {
                val += u_i10x(0 - prec) / 2; /*用于四舍五入,比如1.67保留1位小数为1.7 */
                m = u_ilog10(val);           /*整数位数*/
                if (m < 0)
                    m = 0;
            ...
            { /* E类型 x.xxxxxxe+xx*/
                if (val != 0)
                {
                    val += u_i10x(u_ilog10(val) - prec) / 2; /* 用于四舍五入,prec表示底数部分的小数位数*/
                    e = u_ilog10(val);                       /*整数位数*/
...
int u_vsprintf(char *buf, const char *fmt, va_list args)
{
    ...
    int flags;       /* flags to u_number() */
    int field_width; /* width of output field */
    int precision;   /* min. # of digits for integers; max u_number of chars for from string */
    ...
    repeat:
        ...
        case '-':
            flags |= LEFT;
            goto repeat; /*左对齐-left justify*/
        case '+':
            flags |= PLUS;
            goto repeat; /*p plus with ’+‘*/
        case ' ':
            flags |= SPACE;
            goto repeat; /*p with space*/
        case '#':
            flags |= SPECIAL;
            goto repeat; /*根据其后的转义字符的不同而有不同含义*/
        case '0':
            flags |= ZEROPAD;
            goto repeat; /*当有指定参数时，无数字的参数将补上0*/
        }

        field_width = -1;
        if ('0' <= *fmt && *fmt <= '9')
            field_width = u_skip_atoi(&fmt);
        else if (*fmt == '*')
        {
            ++fmt; /*skip '*' */
        ...
        case 's':
            ...
            if (!s)
                s = "";
            len = u_strnlen(s, precision); /*取字符串的长度，最大为precision*/
            /*非左对齐,左侧填充空格-Satrt*/
            if (!(flags & LEFT))
        ...
        case 'p':
            ...
            str = u_number(str, (unsigned long)va_arg(args, void *), 16, field_width, precision, flags);
            continue;
        ...
        case 'n':
            ...
            {
                unsigned long long *ip = va_arg(args, unsigned long long *);
                *ip = (str - buf);
            }
            // n 整形地址
            else
        ...
        case 'E': /* Floating point (E) */
            char sprint_fe[SZ_NUM_BUF + 1];
            // double数据转字符串
            u_ftoa(sprint_fe, va_arg(args, double), precision, *fmt); /* 浮点转字符串*/
            // 右对齐 左侧补充空格
            if (!(flags & LEFT))
            {
                for (int j = u_strnlen(sprint_fe, SZ_NUM_BUF); j < field_width; j++)
                    *str++ = ' ';
            }
            // 数据主体
            i = 0;
        ...
        str = u_number(str, num, base, field_width, precision, flags);
    }
    *str = '\0';      /*最后在转换好的字符串上加上NULL*/
    return str - buf; /*返回转换好的字符串的长度值*/
}
static void (*xfunc_output)(const char *str, unsigned char len);
...
int u_vprintf(const char *fmt, va_list args)
{
    ...
    int n;
    n = u_vsprintf(sprint_buf, fmt, args);
    if (xfunc_output)
    {
        // 将格式化好的内容输出
        xfunc_output(sprint_buf, n);
    }
    else
    {
