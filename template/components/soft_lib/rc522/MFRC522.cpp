...
{
	gpio_and_delay(_chipSelectPin, LOW); // Select slave
	spi_transfer(reg);					 // MSB == 0 is for writing. LSB is not used in address. Datasheet section 8.1.2.3.
	spi_transfer(value);
	gpio_and_delay(_chipSelectPin, HIGH); // Release slave again
...
)
{
	byte value;
	gpio_and_delay(_chipSelectPin, LOW);  // Select slave
	spi_transfer(0x80 | reg);			  // MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
 ...
 * Initializes the MFRC522 chip.
 */
void MFRC522::PCD_Init(byte (*spi_transfer)(byte),				   // 通信函数
					   byte (*gpio_and_delay)(byte msg, byte val), // gpio及延时函数
					   uint32_t (*millis)()						   // 系统毫秒时间戳
)
{
	this->spi_transfer = spi_transfer;
	this->gpio_and_delay = gpio_and_delay;
	this->millis = millis;
...
{
	gpio_and_delay(_chipSelectPin, HIGH);
	// pinMode(_resetPowerDownPin, OUTPUT);	// Now set the resetPowerDownPin as digital output.
	gpio_and_delay(_resetPowerDownPin, LOW);  // Make sure we have a clean LOW state.
	gpio_and_delay(_delayMs, 20);			  // 8.8.1 Reset timing requirements says about 100ns. Let us be generous: 2μsl
	gpio_and_delay(_resetPowerDownPin, HIGH); // Exit power down mode. This triggers a hard reset.
	// Section 8.8.2 in the datasheet says the oscillator start-up time is the start up time of the crystal + 37,74μs. Let us be generous: 50ms.
	gpio_and_delay(_delayMs, 50);
	...
	while (!uidComplete)
	{
		...
		while (!selectDone)
		{
			...
			{ // All UID bits in this Cascade Level are known. This is a SELECT.
				// Serial.print(("SELECT: currentLevelKnownBits=")); Serial.println(currentLevelKnownBits, 10);
			...
			{ // This is an ANTICOLLISION.
				// Serial.print(("ANTICOLLISION: currentLevelKnownBits=")); Serial.println(currentLevelKnownBits, 10);
...
)
{
	...
	for (int8_t blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--)
	{
		...
		if (firstInGroup)
		{
			// Print access bits
			Serial.print((" [ "));
			Serial.print((g[group] >> 2) & 1, 10);
			Serial.print((" "));
			Serial.print((g[group] >> 1) & 1, 10);
			Serial.print((" "));
			Serial.print((g[group] >> 0) & 1, 10);
			Serial.print((" ] "));
			if (invertedError)
			{
...
void MFRC522::PICC_DumpMifareUltralightToSerial()
{
	...
	{ // Read returns data for 4 pages at a time.
		...
		if (status != STATUS_OK)
		{
			Serial.print(("MIFARE_Read(2) failed: "));
			Serial.println(GetStatusCodeName(status));
			PCD_Init();
			break;
		}
...
bool MFRC522::MIFARE_SetUid(byte *newUid, byte uidSize, bool logErrors)
{
	...
	if (status != STATUS_OK)
	{
		if (logErrors)
		{
			Serial.print(("MIFARE_Read(3) failed: "));
			Serial.println(GetStatusCodeName(status));
