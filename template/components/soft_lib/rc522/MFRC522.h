...
public:
    ...
    /////////////////////////////////////////////////////////////////////////////////////
    void PCD_Init();
    void PCD_Init(byte (*spi_transfer)(byte),                 // 通信函数
    ...
    void read_card(/*char *StudentId, char *StudentName, int Balance_X100*/)
    {
        if (begin() == true)
        {
            MIFARE_Key key;
            ...
            PICC_DumpMifareClassicSectorToSerial(&uid, &key, read_sector);
            status = PCD_Authenticate(PICC_CMD_MF_AUTH_KEY_B, read_sector * 4, &key, &uid);
            if (status != STATUS_OK)
            {
            ...
            status = MIFARE_Write(read_sector * 4 + 2, buffer, 16);
            if (status != STATUS_OK)
            {
            ...
            status = MIFARE_Read(read_sector * 4 + 3, buffer, &byteCount);
            if (status != STATUS_OK)
            {
                Serial.print(("MIFARE_Read() failed: "));
                Serial.println(GetStatusCodeName(status));
    ...
    void register_card(/*char *StudentId, char *StudentName, int Balance_X100*/)
    {
        if (begin() == true)
        {
            MIFARE_Key key;
            ...
            PICC_DumpMifareClassicSectorToSerial(&uid, &key, 1);
            end();
        }
    }
    void pay_card(int money)
    {
    }
    void recharge_card(int money)
    {
    }

protected:
    byte (*spi_transfer)(byte);
    byte (*gpio_and_delay)(byte msg, byte val);
    uint32_t (*millis)();
